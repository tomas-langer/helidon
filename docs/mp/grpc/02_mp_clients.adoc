...
Now when the CDI container instantiates instances of the `ClientVisitor` it will inject a dynamic proxy into the `stringService` field
and then any code in methods in the `ClientVisitor` class can call methods on the `StringService` which will be translated to gRPC calls.
