...
API, Helidon MP uses its own annotation `Path`
...
`"/"`. Note that `Path` is _not_ a bean defining annotation,
thus the need to use `@ApplicationScoped` --which, as before, requires CDI
bean discovery mode to be `annotated`. In addition to `Path`, these
