...
----
<1> Declare `MyWsClient` and later inject it in the constructor.
...
The testing class should be annotated with `@RoutingTest` only if routing tests are required without real port opening. Instead of `MyWsClient`, use `DirectWsClient`.
