...
public class SecurityCdiExtension implements Extension {
    ...
    private void configure(@Observes @Initialized(RuntimeScoped.class) Config config) {
        this.config = config;
        securityBuilder.config(config.get("security"));
    }

    private void registerSecurity(@Observes @Initialized(ApplicationScoped.class) Object adv, BeanManager bm) {
