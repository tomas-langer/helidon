...
import io.helidon.microprofile.cdi.RuntimeScoped;
...
public class ServerCdiExtension implements Extension {
    ...
    private void prepareRuntime(@Observes @Initialized(RuntimeScoped.class) Config config) {
        serverConfigBuilder.config(config.get("server"));
        this.config = config;
    }

    private void startServer(@Observes @Priority(500000) @Initialized(ApplicationScoped.class) Object event,
    ...
    private void stopServer(@Observes @Priority(1) @BeforeDestroyed(ApplicationScoped.class) Object event) {
        if (null == webserver) {
            // nothing to do
            return;
        }
