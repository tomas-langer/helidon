...
public abstract class ServiceProviderBase<T>
    ...
    protected static class ServiceInjectBinderImpl implements ServiceInjectionPlanBinder.Binder {
        ...
        public ServiceInjectionPlanBinder.Binder bind(Ip injectionPoint, boolean useProvider, ServiceInfo serviceInfo) {
            ServiceProvider<?> serviceProvider = BoundServiceProvider.create(services.serviceProvider(serviceInfo), injectionPoint);
            if (useProvider) {
        ...
        public ServiceInjectionPlanBinder.Binder bindOptional(Ip injectionPoint,
            ...
            if (serviceInfos.length == 0) {
                ...
                ServiceProvider<?> serviceProvider = BoundServiceProvider.create(services.serviceProvider(serviceInfos[0]), injectionPoint);
                if (useProvider) {
        ...
        @Override
        public ServiceInjectionPlanBinder.Binder runtimeBindOptional(Ip injectionPoint, boolean useProvider, Class<?> serviceType) {
            if (self instanceof InjectionResolver ir) {
                Optional<Ip> foundIp = self.dependencies()
                        .stream()
                        .filter(it -> it == injectionPoint)
        ...
        @Override
        public ServiceInjectionPlanBinder.Binder runtimeBindNullable(Ip injectionPoint, boolean useProvider, Class<?> serviceType) {
            Optional<? extends Supplier<?>> serviceProvider = services.find(serviceType);
